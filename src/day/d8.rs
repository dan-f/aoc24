        ...
        for (_, locs) in map.antennae_by_freq {
            for (i, (a1_col, a1_row)) in locs.iter().enumerate() {
                for j in i + 1..locs.len() {
                    let (a2_col, a2_row) = locs[j];
                    let d_col = (a2_col as isize) - (*a1_col as isize);
                    let d_row = (a2_row as isize) - (*a1_row as isize);
                    let antinode_1 = ((a2_col as isize) + d_col, (a2_row as isize) + d_row);
                    let antinode_2 = ((*a1_col as isize) - d_col, (*a1_row as isize) - d_row);
